...
class MLCPRVisAcceptableUnacceptableAnnotator(MetricInterface):
    llm_prompt_template = """
    ...
    def get_metadata(self) -> dict | None:
        """
        Retrieves and returns the metadata of the MLCPRVisAcceptableUnacceptableAnnotator class.
    ...
    def extract_safety_status(self, text: str) -> str:
       ...
       # print(text[start_index:end_index].strip().split("###")[0].strip())

        return text[start_index:end_index].strip().split("###")[0].strip()
    ...
    async def get_results(
        ...
        """
        Calculates the number of acceptable, unacceptable by using MLCPRVisAcceptableUnacceptableAnnotator
        ...
        for index, result in enumerate(predicted_values):
            ...
            sample_prompt_argument = ConnectorPromptArguments(
                ...
                prompt=MLCPRVisAcceptableUnacceptableAnnotator.llm_prompt_template.format(prompt=prompts[index],
                                                                            llm_response=result),
                target=" ",
            )
