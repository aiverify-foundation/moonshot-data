...
class MLCPRV(MetricInterface):
    ...
    def get_metadata(self) -> dict | None:
        """
        Retrieves and returns the metadata of the MLCPRV class.

        Returns:
            dict | None: A dictionary containing the 'id', 'name', 'description', 'endpoints' and 'configurations'
            of the MLCPRV class, or None if not applicable.
        """
        return {
            "id": self.id,
            "name": self.name,
    ...
    async def get_results(
        ...
        """
        Calculates the number of acceptable, unacceptable by using MLCPRV
        ...
        for index, result in enumerate(predicted_values):
            sample_prompt_argument = ConnectorPromptArguments(
                prompt_index=index,
                prompt=MLCPRV.llm_prompt_template.format(prompt=prompts[index],
                                                                            llm_response=result),
                target=" ",
            )
